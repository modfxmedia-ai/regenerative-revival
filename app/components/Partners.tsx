    ...
    <section id="partners" className="relative py-28 bg-secondary overflow-hidden">
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8 }} className="text-center max-w-3xl mx-auto mb-20">
          <span className="text-sm font-semibold uppercase tracking-widest text-primary-light">For Medical Practices & Businesses</span>
          <h2 className="mt-4 text-4xl sm:text-5xl font-bold leading-tight text-white">Partner With Regenerative Revival</h2>
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-16">
          {perks.map((p, i) => (
            <motion.div key={p.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 + i * 0.1 }} className="rounded-3xl p-8 bg-white/10 backdrop-blur-sm border border-white/10 hover:bg-white/15 transition-all group">
        ...
        <motion.div initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.8, delay: 0.5 }} className="rounded-3xl bg-white p-8 sm:p-12 flex flex-col sm:flex-row items-center justify-between gap-8">
          <div>
            <h3 className="text-2xl sm:text-3xl font-bold text-gray-900">Ready to grow your practice?</h3>
            <p className="mt-2 text-gray-600 max-w-lg">Join our network of medical partners and start offering cutting-edge stem cell therapy with full support and shared revenue.</p>
          </div>
          <a href="#contact" className="group shrink-0 flex h-14 items-center gap-2 rounded-full bg-primary px-8 text-base font-semibold text-white transition-all hover:bg-primary-dark hover:shadow-xl hover:shadow-primary/20">
            Become a Partner
