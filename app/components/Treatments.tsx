    ...
    <section id="treatments" className="relative py-28 bg-cream overflow-hidden">
      ...
      <div ref={ref} className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
          {benefits.map((b, i) => (
            <motion.div key={b.title} initial={{ opacity: 0, y: 30 }} animate={inView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.6, delay: 0.1 + i * 0.1 }} className="group bg-white rounded-3xl p-8 soft-shadow hover:soft-shadow-lg transition-all hover:-translate-y-1 border border-gray-100">
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, x: -30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.3 }}>
            ...
            <div className="grid grid-cols-2 gap-4">
              {conditions.map((c, i) => (
                <motion.div key={c.label} initial={{ opacity: 0, x: -20 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.5 + i * 0.1 }} className="flex items-center gap-3 bg-white rounded-xl p-4 soft-shadow border border-gray-100 hover:border-primary/20 transition-colors">
                  <c.icon className="h-5 w-5 text-primary shrink-0" />
                  <span className="text-sm font-medium text-gray-900">{c.label}</span>
                </motion.div>
              ))}
            </div>
            <a href="#contact" className="inline-flex items-center gap-2 mt-8 text-primary font-semibold hover:text-primary-dark transition-colors">
              See if you qualify →
            </a>
          </motion.div>

          <motion.div initial={{ opacity: 0, x: 30 }} animate={inView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.8, delay: 0.4 }} className="relative rounded-3xl overflow-hidden soft-shadow-lg">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img src="https://images.pexels.com/photos/4033148/pexels-photo-4033148.jpeg?auto=compress&cs=tinysrgb&w=800" alt="Stem cell therapy treatment" className="w-full h-auto rounded-3xl object-cover aspect-video" />
